...
void salvaImagem(Imagem *img, const char *nomeArquivo){
    ...
    fprintf(arquivo, "P3\n");
    fprintf(arquivo, "#EP1 - Estrutura de Dados 1 - Talisson Sozinho da Costa\n");
    fprintf(arquivo, "%d %d %d\n", obtemLargura(img), obtemAltura(img), 255);
    ...
    for(int i=0;i< obtemAltura(img);i++){
		for(int j=0;j<obtemLargura(img); j++){
			fprintf( arquivo, "%d\n", obtemPixel(img, i, j).cor[RED] );
			fprintf( arquivo, "%d\n", obtemPixel(img, i, j).cor[GREEN] );
			fprintf( arquivo, "%d\n", obtemPixel(img, i, j).cor[BLUE] );
		}
	}
